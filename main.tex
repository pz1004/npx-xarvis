...
\item A Numba JIT-optimized software implementation achieves over 5M events/sec throughput, demonstrating real-time viability on standard CPUs.
\item Empirical evaluation using the Event Structural Ratio (ESR) confirms that the proposed confidence-coded pipeline preserves spatial structure while achieving superior noise discrimination.
...
In our software implementation, we utilize Numba JIT compilation to accelerate the core filtering loop. This optimization yields an 88.5$\times$ speedup over pure Python, achieving a real-time processing throughput of over 5 million events per second (5M eps) on a standard CPU, demonstrating the practical efficiency of the proposed architecture even without specialized hardware.
...
On top of that repair, the paper provided a complete six-stage event-native pipeline with three strengthened analytical results---a self-consistent BA false-accept bound with an explicit stability criterion $M\lambda_n\Delta t\,(1-q_n)<1$, a leading-order shot-noise rejection ratio $r(\gamma)\approx \mu_s^\gamma\,K_0!/(K_0+\gamma)!$, and a DVS-specialized cascade-ordering criterion---together with a hardware-aware mapping across balanced, ultra-low-memory, and ultra-low-latency regimes and an analytical per-event throughput bound. We addressed the inherent cold-start problem of split-state designs via a warm-up seeding mechanism, and demonstrated the practical viability of the approach with a Numba JIT-optimized software implementation achieving over 5M eps throughput. The hot-pixel recovery mechanism ensures that transient anomalies do not permanently disable pixels, and the cleaned hot-pixel bookkeeping in Algorithm~\ref{alg:pipeline} keeps the pixel-isolation detector aligned with its intent. Furthermore, empirical evaluations confirm that the proposed confidence-coded pipeline achieves superior noise discrimination (AUC) across diverse noise regimes while effectively preserving spatial structure (ESR).
