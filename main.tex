...
\item A warm-up seeding mechanism resolves the inherent cold-start bootstrap problem of split-state designs, enabling robust initialization of the accepted history.
\item A Numba JIT-optimized software implementation is accompanied by timestamped throughput benchmarking so preprocessing speed can be reported from reproducible artifacts.
\item The evaluation protocol reports the Event Structural Ratio (ESR) alongside AUC so signal retention and noise discrimination can be assessed together.
...
\begin{keywords}
Dynamic Vision Sensor \sep event camera \sep spiking neural network \sep event-domain denoising \sep neuromorphic preprocessing \sep background activity noise \sep split-state semantics \sep confidence coding \sep warm-up seeding \sep Event Structural Ratio
\end{keywords}
...
\item \textbf{Hardware-aware architecture, throughput analysis, and evaluation protocol.} We present a complete six-stage event-native pipeline with confidence-coded SNN ingestion, map the same logical design to balanced, ultra-low-memory, and ultra-low-latency hardware variants, provide an analytical per-event throughput estimate, and specify a reproducible two-level evaluation protocol. We also identify and solve the split-state cold-start problem via warm-up seeding, and provide a Numba JIT-optimized implementation with timestamped throughput benchmarking. The empirical protocol includes structural preservation metrics (ESR) alongside traditional classification accuracy.
\end{enumerate}
...
A parallel line of work uses learned models. \citet{baldwin2020epm} train a convolutional network (EDnCNN) to classify events as signal or noise; \citet{benmiled2024adaptive} propose an adaptive unsupervised spatiotemporal filter; and \citet{riosnavarro2023mlpf} deploy a lightweight MLP denoiser inside camera-adjacent digital logic. More recently, \citet{zhao2024pclf} introduced a hardware-friendly partitioned cache-like spatiotemporal filter (PCLF), and \citet{ruan2025edmamba} proposed EDmamba, a spatiotemporal decoupled state space model for efficient event denoising. Furthermore, the evaluation of such filters has evolved beyond simple classification accuracy; benchmarks like E-MLB~\citep{ding2023emlb} emphasize structural preservation metrics such as the Event Structural Ratio (ESR). While these methods can outperform hand-crafted rules when labeled data is available, they introduce training dependencies and model-size constraints that complicate direct integration with SNNs and hardware deployment.
...
\textbf{The Cold-Start Bootstrap Problem and Warm-up Seeding.}
A fundamental challenge inherent to the split-state design is the \emph{cold-start} problem. Because support is calculated exclusively from the accepted history $\mathcal{M}^{\text{acc}}$, if the accepted history is initially empty, no event can ever receive support. Consequently, if $K_0 \geq 1$, all events are rejected, and the accepted history remains permanently empty. To resolve this bootstrap failure, we introduce a \emph{warm-up seeding} mechanism. During an initial warm-up period (e.g., the first few milliseconds of the stream), the filter operates with $K_0 = 0$ (or equivalently, falls back to the `Proposed +REF` logic), allowing initial valid events to populate $\mathcal{M}^{\text{acc}}$. Once the accepted history is seeded, the filter transitions to the strict $K_0 \geq 1$ threshold.
...
\textbf{Analytical and Software Throughput.}
...
In our software implementation, we use Numba JIT compilation to accelerate the core filtering loop and provide a timestamped benchmarking script for preprocessing throughput. The benchmark records the dataset, method, selected filter parameters, event count, sample count, median per-sample latency, aggregate throughput, software versions, and hardware metadata. Exact throughput values are therefore reported from saved artifacts rather than from informal console measurements.
...
\begin{itemize}
    ...
    \item \textbf{Q6:} Does the proposed pipeline preserve the spatial structure of the signal stream, as measured by the Event Structural Ratio (ESR), better than baseline methods across noise types and ratios?
\end{itemize}
...
\midrule
Software stack & Python, PyTorch, snnTorch, Tonic, NumPy, Numba (JIT-compiled filter loop), pandas, scikit-learn, matplotlib, seaborn \\
...
Tuning grids & BA/STCF: $\Delta t\in\{1000,2000,5000\}$; Proposed +REF: $\tau_{\text{ref,dig}}\in\{500,1000,2000\}$; Proposed +SUP/+OPP/+CONF: $\tau_{\text{ref,dig}}\in\{500,1000,2000\}$, $\Delta t\in\{1000,2000,5000\}$, $K_0\in\{1,2\}$, $\gamma\in\{0,1,2\}$, $\tau_{\text{pair}}\in\{500,1000,2000\}$, $K_{\text{high}}\in\{2,3,4\}$, $T_{\text{warm}}\!=\!5000\,\mu$s \\
...
\label{eq:ekr}
\end{equation}
Furthermore, to quantify the preservation of spatial structure after denoising, we adopt the Event Structural Ratio (ESR) metric from the E-MLB benchmark~\citep{ding2023emlb}:
\begin{equation}
\text{ESR} = \frac{|\mathcal{E}_{\text{kept}} \cap \mathcal{E}_{\text{sig}}|}{|\mathcal{E}_{\text{sig}}|},
\label{eq:esr}
\end{equation}
where $\mathcal{E}_{\text{sig}}$ is the set of ground-truth signal events. ESR measures the fraction of true signal events that are retained after filtering, directly quantifying the structural preservation of the denoised stream. Unlike EKR, which measures the total retention ratio, ESR focuses specifically on the signal component, providing a more discriminative evaluation of spatial structure preservation.
...
\label{eq:aunc}
\end{equation}
where $\text{Acc}(\rho)$ is the test accuracy after injecting a noise ratio $\rho$. Similarly, the mean ESR across noise ratios is reported as a structural robustness indicator:
\begin{equation}
\overline{\text{ESR}} = \frac{1}{|\mathcal{R}|}\sum_{\rho\in\mathcal{R}} \text{ESR}(\rho).
\label{eq:mean_esr}
\end{equation}
...
On top of that repair, the paper provided a complete six-stage event-native pipeline with three strengthened analytical results---a self-consistent BA false-accept bound with an explicit stability criterion $M\lambda_n\Delta t\,(1-q_n)<1$, a leading-order shot-noise rejection ratio $r(\gamma)\approx \mu_s^\gamma\,K_0!/(K_0+\gamma)!$, and a DVS-specialized cascade-ordering criterion---together with a hardware-aware mapping across balanced, ultra-low-memory, and ultra-low-latency regimes and an analytical per-event throughput bound. We addressed the inherent cold-start problem of split-state designs via a warm-up seeding mechanism and provide software support for artifact-backed throughput measurement. The hot-pixel recovery mechanism ensures that transient anomalies do not permanently disable pixels, and the cleaned hot-pixel bookkeeping in Algorithm~\ref{alg:pipeline} keeps the pixel-isolation detector aligned with its intent. The accompanying evaluation protocol specifies how future empirical runs should report noise discrimination (AUC), signal retention (ESR), downstream accuracy, and efficiency from saved artifacts.
